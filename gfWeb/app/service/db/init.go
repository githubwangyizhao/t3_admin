...
func initCharge() {
	...
	//数据库用户名
	dbUser := g.Cfg().GetString("database.charge.user")
	//数据库密码
	dbPwd := g.Cfg().GetString("database.charge.pass")
	//数据库IP
	dbHost := g.Cfg().GetString("database.charge.host")
	//数据库端口
	dbPort := g.Cfg().GetString("database.charge.port")
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", dbUser, dbPwd, dbHost, dbPort, dbName)
	var err error
